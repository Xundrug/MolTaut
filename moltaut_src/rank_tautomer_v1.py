...
from src.get_vmrs import enumerate_vmrs
import warnings
...
def calc_solv(tauts, fmax, num_confs, is_fragment):
    if is_fragment:
        ...
        res = []
        for idx, tsmi in enumerate(tauts_smis_exclude_linker):
            solv, dE = predict_by_smi(tsmi, fmax,  num_confs)
            lsmi = tauts_smis_include_linker[idx]
            res.append([lsmi, solv, dE])
    else:
        tauts_smis = [taut.smi for taut in tauts]
        res = []
        for tsmi in tauts_smis:
            solv, dE = predict_by_smi(tsmi, fmax,  num_confs)
